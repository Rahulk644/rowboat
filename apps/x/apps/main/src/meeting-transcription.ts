...
const ECHO_MAX_BOUNDARY_DELTA_SAMPLES = 1_600; // 100 ms at the canonical 16 kHz clock.
...
function boundedTokenEditDistance(left: readonly string[], right: readonly string[], limit: number): number {
...
function hasStrongEchoTextMatch(mic: MeetingTranscriptSegment, system: MeetingTranscriptSegment): boolean {
  ...
  return boundedTokenEditDistance(micTokens, systemTokens, ECHO_MAX_TOKEN_EDITS) <= ECHO_MAX_TOKEN_EDITS;
}

function hasStrongEchoTimingMatch(mic: MeetingTranscriptSegment, system: MeetingTranscriptSegment): boolean {
...
function isEligibleMicEchoCandidate(segment: MeetingTranscriptSegment): boolean {
...
export type CrossChannelEchoSuppression = {
  suppressedMicSegmentIds: string[];
  systemUpserts: MeetingTranscriptSegment[];
};

/**
 * Conservatively collapse a microphone loopback copy into its one matching
 * system segment. This deliberately requires a one-to-one graph: if either
 * side has multiple plausible matches, it leaves every segment untouched.
 * It never tries to identify speakers or suppresses short/common utterances.
 */
export function findCrossChannelEchoSuppressions(
  segments: Iterable<MeetingTranscriptSegment>,
): CrossChannelEchoSuppression {
  const all = [...segments];
  const mics = all.filter(isEligibleMicEchoCandidate);
  const systems = all.filter(isEligibleSystemEchoCandidate);
  const matchesByMic = new Map<string, MeetingTranscriptSegment[]>();
  const matchesBySystem = new Map<string, MeetingTranscriptSegment[]>();

  for (const mic of mics) {
    const matches = systems.filter((system) => (
      hasStrongEchoTimingMatch(mic, system) && hasStrongEchoTextMatch(mic, system)
    ));
  ...
  for (const mic of mics) {
    ...
    suppressedMicSegmentIds.push(mic.segmentId);
    suppressedBySystem.set(system.segmentId, [mic.segmentId]);
  }
  ...
  private adaptSnapshot(
    ...
    if (echoSuppression.suppressedMicSegmentIds.length > 0) {
      for (const segmentIdValue of echoSuppression.suppressedMicSegmentIds) {
        active.segments.delete(segmentIdValue);
        active.pendingAttributionUpserts.delete(segmentIdValue);
      }
    ...
    const survived = (segment: MeetingTranscriptSegment): boolean => (
      active.segments.get(segment.segmentId)?.revision === segment.revision
    );
    const emitted = mergeMeetingTranscriptSegments([], [
      ...accepted.filter(survived),
      ...attributed.filter(survived),
      ...echoSuppression.systemUpserts,
      ...pending,
    ]);
